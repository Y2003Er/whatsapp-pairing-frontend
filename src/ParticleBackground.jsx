...
export default function ParticleBackground() {
  ...
  useEffect(() => {
    ...
    let frame; let active = !document.hidden; let width = 0; let height = 0; let dpr = 1; let network; let starfall; let last = performance.now();
    ...
    const draw = (time) => { if (!active) return; const step = Math.min(2.5, (time - last) / 16.667); last = time; ctx.clearRect(0, 0, width, height); ctx.shadowBlur = 0; currentTheme() === "warmStone" ? drawStarfall(time, step) : drawNetwork(time, step); ctx.shadowBlur = 0; frame = requestAnimationFrame(draw); };
    const onVisibility = () => { active = !document.hidden; if (active) { last = performance.now(); frame = requestAnimationFrame(draw); } else cancelAnimationFrame(frame); };
