...
import {
  ...
  ShieldCheck, Smartphone, Trash2, Wifi, WifiOff, X,
} from "lucide-react";
...
function FleetHealth({ stats }) {
  const segments = [{ key: "online", label: "Online", color: "#22c55e" }, { key: "connecting", label: "Connecting", color: "#f59e0b" }, { key: "offline", label: "Offline", color: "#94a3b8" }, { key: "loggedOut", label: "Logged out", color: "#f43f5e" }];
...
function AdminWorkspace({ onLogout }) {
  ...
  const [path, setPath] = useState(currentPath); const [menuOpen, setMenuOpen] = useState(false); const [stats, setStats] = useState(null); const [bots, setBots] = useState([]); const [loading, setLoading] = useState(true); const [updatedAt, setUpdatedAt] = useState(null);
  const navigate = useCallback((next) => { window.history.pushState({}, "", next); setPath(next); }, []);
  const load = useCallback(async () => { try { const data = await api("/bots"); setStats(data.stats); setBots(data.instances || []); setUpdatedAt(new Date()); } catch (error) { toast(error.message); if (/unauthor/i.test(error.message)) onLogout(); } finally { setLoading(false); } }, [onLogout]);
