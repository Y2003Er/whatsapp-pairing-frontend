...
function BusinessTable({ title, description, endpoint, rowsKey, columns, renderActions, extraToolbar, modals, searchable, searchKey = "phoneNumber", searchPlaceholder = "Search phone number" }) {
  ...
  const visibleRows = searchable && query ? rows.filter((row) => String(row[searchKey] || "").replace(/\D/g, "").includes(query)) : rows;
  return <><div className="cc-page-heading"><div><p className="cc-eyebrow">BUSINESS OPERATIONS</p><h1>{title}</h1><p>{description}</p></div><div style={{ display: "flex", gap: 10 }}>{extraToolbar?.(load)}<button className="cc-secondary" onClick={load}><RefreshCw size={16} />Refresh</button></div></div>{searchable && <section className="cc-toolbar"><label><Search size={17} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder={searchPlaceholder} /></label><span>{visibleRows.length} of {rows.length}</span></section>}<section className="cc-panel cc-data-panel">{loading ? <div className="cc-loading"><Loader2 className="cc-spin" /> Loading records…</div> : error ? <div className="cc-empty"><h2>Could not load data</h2><p>{error}</p><button className="cc-secondary" onClick={load}>Retry</button></div> : visibleRows.length ? <div className="cc-data-table"><div className="cc-data-head" style={{ "--cols": allColumns.length }}>{allColumns.map((column) => <span key={column.label}>{column.label}</span>)}</div>{visibleRows.map((row, index) => <div className="cc-data-row" style={{ "--cols": allColumns.length }} key={row.transactionId || row.paymentReference || row.ownerId || row.id || index}>{columns.map((column) => <span key={column.label}>{column.render ? column.render(row) : row[column.key] ?? "—"}</span>)}{renderActions && <span className="cc-row-actions">{renderActions(row, load)}</span>}</div>)}</div> : <div className="cc-empty"><h2>{searchable && query ? "No matching records" : "No records yet"}</h2><p>{searchable && query ? "Try a different phone number." : "This section displays real platform data when it becomes available."}</p></div>}</section>{modals}</>;
}
...
function SubscriptionsView() {
  ...
  return <BusinessTable key={key} title="Subscriptions" description="Live records from the existing platform database. Use Manage to override a plan, extend access, or suspend an account." endpoint="/admin/subscriptions" rowsKey="subscriptions" columns={columns}
    searchable searchKey="phoneNumber" searchPlaceholder="Search phone number"
