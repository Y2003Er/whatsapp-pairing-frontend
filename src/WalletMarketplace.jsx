...
// "Pricing coming soon" is meant for packages awaiting a price. A usage
// transaction (coins spent on a feature) legitimately has no money amount —
// showing that packages-only placeholder there implies a price is coming,
// which is misleading. Coin count is already shown in its own column.
function formatTransactionAmount(transaction) {
  if (transaction.type === "usage") return "—";
...
function packageFeatureLabel(feature) {
  const name = typeof feature === "string" ? feature : feature?.name;
  return String(name || "").replace(/_/g, " ").toLowerCase().replace(/\b\w/g, (letter) => letter.toUpperCase());
}

function PackageFeatures({ features }) {
  if (!Array.isArray(features) || features.length === 0) return null;
  return <div className="wallet-package-features" aria-label="Included features">
    <span className="wallet-package-features-title">Included features</span>
    {features.map((feature) => <span key={typeof feature === "string" ? feature : feature.name}><Check size={12} /> {packageFeatureLabel(feature)}</span>)}
  </div>;
}

const PACKAGE_BADGES = {
  RECOMMENDED: { label: "Recommended", className: "is-recommended" },
  POPULAR: { label: "Popular", className: "is-popular" },
  BEST_VALUE: { label: "Best Value", className: "is-best-value" },
};

function PackageBadge({ badge }) {
  const meta = PACKAGE_BADGES[String(badge || "").toUpperCase()];
  if (!meta) return null;
  return <span className={`wallet-package-badge ${meta.className}`}>{meta.label}</span>;
}
...
function PurchaseModal({ selected, phoneNumber, phoneError, onPhoneChange, onPhoneBlur, onClose, onConfirm, pending }) {
  ...
  return <div className="wallet-modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <section className="wallet-modal" role="dialog" aria-modal="true" aria-labelledby="purchase-title">
      ...
      <label className="wallet-phone-field"><span>Mobile Money Number (HaloPesa, Airtel Money, Mix by Yas)</span><input type="tel" inputMode="tel" autoComplete="tel" placeholder="07XXXXXXXX" value={phoneNumber} onChange={(event) => onPhoneChange(event.target.value)} onBlur={onPhoneBlur} aria-invalid={Boolean(phoneError)} aria-describedby={phoneError ? "mobile-number-error" : undefined} disabled={pending} /></label>
...
export default function WalletMarketplace({ onNavigate }) {
  ...
  const loadTransactions = useCallback(async () => {
    ...
    try {
      const data = await getTransactions(session.token, { page: String(page), limit: String(PAGE_SIZE), sortBy: "createdAt", sortOrder: "desc", type: "purchase" });
...
function WalletStyles() { return <style>{`
  ...
  .wallet-section { margin-top: 44px; }.wallet-section-head { margin-bottom: 18px; align-items: end; }.wallet-section h2 { margin: 0; font: 750 clamp(1.18rem, 3vw, 1.6rem)/1.15 var(--font-display); letter-spacing: -.025em; }.wallet-trust { display: flex; align-items: center; gap: 6px; color: var(--token-success); background: var(--token-success-bg); padding: 7px 10px; border-radius: 999px; font-size: .7rem; font-weight: 700; white-space: nowrap; }.wallet-packages { display: grid; grid-template-columns: repeat(auto-fit, minmax(205px, 1fr)); gap: 13px; }.wallet-package { min-height: 245px; padding: 19px; display: flex; flex-direction: column; transition: transform .23s ease, border-color .23s ease, box-shadow .23s ease; }.wallet-package:hover, .wallet-package.is-selected { transform: translateY(-5px); border-color: var(--token-border-strong); box-shadow: 0 18px 46px var(--token-glow); }.wallet-package-top { min-height: 26px; display: flex; align-items: start; justify-content: space-between; gap: 8px; }.wallet-package-name { color: var(--token-info); font-size: .74rem; font-weight: 800; text-transform: uppercase; letter-spacing: .06em; }.wallet-package-chips { display: flex; align-items: center; gap: 6px; flex-wrap: wrap; justify-content: flex-end; }.wallet-saving { color: var(--token-success); background: var(--token-success-bg); padding: 4px 7px; border-radius: 99px; font-size: .63rem; font-weight: 800; }.wallet-package-badge { padding: 4px 7px; border-radius: 99px; font-size: .63rem; font-weight: 800; white-space: nowrap; }.wallet-package-badge.is-recommended { color: var(--token-info); background: var(--token-info-bg); }.wallet-package-badge.is-popular { color: var(--token-warning); background: var(--token-warning-bg); }.wallet-package-badge.is-best-value { color: var(--token-on-accent); background: var(--token-accent-fill); }.wallet-package-credits { font-size: 2rem; letter-spacing: -.05em; margin-top: 27px; }.wallet-package-credits small { color: var(--token-muted); font-size: .7rem; letter-spacing: 0; }.wallet-package-price { color: var(--token-muted); font-size: .82rem; margin: 3px 0 12px; }.wallet-package-features { display: grid; gap: 5px; margin: 0 0 16px; color: var(--token-text-muted); font-size: .67rem; line-height: 1.3; }.wallet-package-features-title { color: var(--token-muted); font: 800 .6rem var(--font-mono); letter-spacing: .07em; text-transform: uppercase; }.wallet-package-features > span:not(.wallet-package-features-title) { display: flex; align-items: center; gap: 5px; }.wallet-package-features svg { flex: 0 0 auto; color: var(--token-success); }.wallet-package-button, .wallet-primary-button, .wallet-secondary-button { display: inline-flex; justify-content: center; align-items: center; gap: 7px; border-radius: 11px; padding: 11px 13px; border: 1px solid transparent; font-size: .76rem; font-weight: 800; cursor: pointer; transition: transform .2s ease, opacity .2s ease; }.wallet-package-button { width: 100%; margin-top: auto; background: var(--token-surface-strong); border-color: var(--token-card-border); color: var(--token-text); }.wallet-package-button:hover, .wallet-primary-button:hover { transform: translateY(-2px); }.wallet-primary-button { background: var(--token-accent-fill); color: var(--token-on-accent); box-shadow: 0 8px 24px var(--token-glow); }.wallet-secondary-button { background: transparent; border-color: var(--token-card-border); color: var(--token-text); }.wallet-package-skeleton { pointer-events: none; }.wallet-skeleton-chip { width: 45%; height: 13px; }.wallet-skeleton-title { width: 74%; height: 31px; margin-top: 32px; }.wallet-skeleton-number { width: 52%; height: 15px; margin-top: 8px; }.wallet-skeleton-button { width: 100%; height: 37px; margin-top: auto; }
