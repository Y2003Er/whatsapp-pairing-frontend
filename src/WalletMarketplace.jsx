...
// Short label + brand-ish color per known provider id, so each button reads
// at a glance without reproducing any provider's actual trademarked logo.
// Unknown/future provider ids still render fine via the fallback.
const PROVIDER_META = {
  MPESA: { short: "M", color: "#3AA13F" },
  AIRTEL_MONEY: { short: "A", color: "#E4002B" },
  HALOPESA: { short: "H", color: "#F7941D" },
  BINANCE_PAY: { short: "B", color: "#F0B90B" },
};
function providerMeta(id) {
  return PROVIDER_META[id] || { short: (id || "?").charAt(0).toUpperCase(), color: "var(--token-info)" };
}
    ...
    <div className="wallet-provider-grid" role="radiogroup" aria-label="Payment provider">
      {providers.map((item) => {
        ...
        return <button
          ...
          className={`wallet-provider-option ${isSelected ? "is-selected" : ""} ${item.configured ? "" : "is-unconfigured"}`}
          onClick={() => onChange(item.id)}
          disabled={disabled}
        >
          <span className="wallet-provider-icon" style={{ background: meta.color }}>{meta.short}</span>
          <span className="wallet-provider-name">{item.name}</span>
...
function PurchaseModal({ selected, providers, provider, onProviderChange, onClose, onConfirm, pending }) {
  ...
  return <div className="wallet-modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <section className="wallet-modal" role="dialog" aria-modal="true" aria-labelledby="purchase-title">
      ...
      <ProviderPicker providers={providers} provider={provider} onChange={onProviderChange} disabled={pending} />
...
export default function WalletMarketplace({ onNavigate }) {
  ...
  const loadSummary = useCallback(async () => {
    ...
    try {
      ...
      setProvider((current) => nextProviders.some((item) => item.id === current) ? current : (nextProviders[0]?.id || ""));
  ...
  const startPurchase = async () => {
    ...
    if (!provider) { toast("No payment provider is available yet.", "error"); return; }
    setPurchasePending(true);
    try {
      const data = await createPurchase(session.token, selected.id, provider);
