...
function ProviderIcon({ meta }) {
  const [failed, setFailed] = useState(false);
  if (meta.logo && !failed) {
    return <span className="wallet-provider-icon wallet-provider-icon-logo"><img src={meta.logo} alt="" onError={() => setFailed(true)} /></span>;
  }
  return <span className="wallet-provider-icon" style={{ background: meta.color }}>{meta.short}</span>;
}
