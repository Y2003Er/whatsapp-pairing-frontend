...
function OwnerOverview({ bot, session, onRefresh, onNavigate }) {
  ...
  return <section className="enterprise-overview" aria-label="Bot overview">
    <div className="enterprise-welcome">
      ...
      <div className="enterprise-profile"><span className="enterprise-avatar"><User size={20} /></span><span><strong>Bot owner</strong><small>+{session.phoneNumber}</small><small>{String(session.subscription?.plan || session.membershipTier || "").replace(/_/g, " ")}</small><small>{session.subscription ? `${session.subscription.status} · ${session.subscription.remainingDays} days remaining` : ""}</small></span><span className="dash-status-pill" style={{ color: status.color, background: status.bg }}>{status.label}</span></div>
    </div>
    ...
    <div className="enterprise-grid">
      ...
      <section className="enterprise-panel"><div className="enterprise-panel-heading"><div><span>Subscription</span><h3>{String(subscription?.plan || "Free").replace(/_/g, " ")}</h3></div></div><div className="activity-timeline"><div><span><strong>{subscription?.status || "Unavailable"}</strong><small>{subscription?.status === "TRIAL" ? "Trial access" : "Current subscription status"}</small></span><time>{subscriptionExpiry ? new Intl.DateTimeFormat(undefined, { dateStyle: "medium" }).format(new Date(subscriptionExpiry)) : "No expiry"}</time></div><div><span><strong>{subscription?.remainingDays ?? 0} days remaining</strong><small>Subscription time available</small></span></div>{subscription?.scheduledPlan && <div><span><strong>Upcoming: {String(subscription.scheduledPlan).replace(/_/g, " ")}</strong><small>Starts after your trial</small></span><time>{subscription.scheduledStartDate ? new Intl.DateTimeFormat(undefined, { dateStyle: "medium" }).format(new Date(subscription.scheduledStartDate)) : "After trial"}</time></div>}</div><button type="button" className="enterprise-text-action" onClick={() => onNavigate?.("coinshop")}>Upgrade / Manage Plan</button></section>
